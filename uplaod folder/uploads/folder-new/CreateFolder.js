...
export default function CreateFolder({
  ...
  const fetchFolders = async () => {
    try {
      const url = `http://127.0.0.1:8005/allFolders/${templateId}`;
  ...
  const createFolderAPI = (newFolderPath) => {
    return axios
      .get(`http://localhost:8005/createFolder/?path=uploads/FolderTemplates/${templateId}/${newFolderPath}&foldername=${newFolderName}`)
      .then((response) => {
